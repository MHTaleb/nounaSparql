...
public class Client {
    ...
    public void selectByFullName(String firstName, String lastName) {
        ...
        String query = "PREFIX  ex:   <http://www.semanticweb.org/home/ontologies/2016/10/ex#>"
                ...
                + " OPTIONAL"
                + "    { ?firstName  ex:hasValue  ?o } ." // o la variable qui as le firstName 
        ...
        Query qry = QueryFactory.create(query);  // creer la requete
        readOnthologie("src/Prv.owl"); // specifie l ontologie de lecture ( bdd)
        QueryExecution exe = QueryExecutionFactory.create(qry, ontModel); // creer l objet qui va executer la requete
        ResultSet rs = exe.execSelect(); // executé requete et recuperer le resultat dans un ResultSet
        System.out.println("begin");
        while (rs.hasNext()) { // tanque on a un resultat
            QuerySolution querySolution = (QuerySolution) rs.next(); // obtenir le resultat courant
            try {
                // recuperer le first name du resultat courant
                // querySolution.get("o") est la methode qui permet de recuper le contenu de la variable o dans la requete
                String myFirstName = "" + querySolution.get("o").toString().replace("http://www.semanticweb.org/home/ontologies/2016/10/ex#", "ex:");
                // recuperer le LastName depuis le resultat de la requete
                ...
                if (myFirstName.toLowerCase().equals(firstName.toLowerCase()) && myLastName.toLowerCase().equals(lastName.toLowerCase())) {
                    ...
                    // fin calcul de l age
                    if (age < 25 || age > 75) {
                        JOptionPane.showMessageDialog(null, "To rent a vehicle your age must be\nbetween [25,75]");
                    } else // je teste l age entre 25 75 sinon
                    if (age < 18) {
                        JOptionPane.showMessageDialog(null, "people under 18 years old doesn't have a driver license");
                        return;
                    } else {
                        // je test s il est inferieur a 18
                        //sinon  je calcule la validité du permis
                        ...
                        // fin du calcule de la validité du permis  ( en mois )
                        if (currentPassedTime < 12) {
                            // si le permis a moins de 12 mois j affiche erreur
                            JOptionPane.showMessageDialog(null, "Period of validity of a driver's license\nmust be at least 12 months.");
                            return;
                        } else {
                            // si je suis ici cela veut dire aucune anomalie n as ete detecter alors je dit qu il a le bon age et le bon permi
                            JOptionPane.showMessageDialog(null, "Your age is accepted : you'r allowed to rent a vehicle");
                            return;
                        }
                        
                        // puis je passe a la prochaine ligne dans la bdd si la precente n est pas mon client souhaité
                    }
